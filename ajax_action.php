	...
	if($_POST['page'] == 'view_subject_exam')
	{
		if($_POST['action'] == 'load_question')
		{
			...
			foreach($result as $row)
			{
				$output .= '
				<div class="card">
					<div class="card-header bg-success" style="font-size: 18px; color: #FFFFFF"><b>Question: </b>'.$row["exam_subject_question_title"].'</div>
					<div class="card-body">
						<div class="row">
				';
				...
				$output .= '
				  	<div align="center">
				   		<button type="button" name="previous" class="btn btn-info btn-sm previous" id="'.$previous_id.'" '.$if_previous_disable.'><i class="fa fa-chevron-left"></i> Previous</button>
				   		<button type="button" name="next" class="btn btn-warning btn-sm next" id="'.$next_id.'" '.$if_next_disable.'> Next <i class="fa fa-chevron-right"></i></button>
				  	</div>
				  	</div></div>';
			}
		...
		if($_POST['action'] == 'question_navigation')
		{
			...
			$output = '
			<div class="card">
				<div class="card-header bg-success text-white"><b>Question Navigation</b></div>
				<div class="card-body">
					<div class="row">
			';
			$count = 1;
			foreach($result as $row)	
			{
				$output .= '
				<div class="mb-2 mr-2">
					<button type="button" class="btn btn-success question_navigation" data-question_id="'.$row["exam_subject_question_id"].'">'.$count.'</button>
				</div>
				';
				$count++;
			}
