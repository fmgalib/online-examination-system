...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '
			<div align="center">
			<button type="button" name="edit_button" class="btn btn-warning  btn-sm edit_button" data-id="'.$row["student_id"].'"><i class="fas fa-edit"></i></button>
			</div>
			';
			$data[] = $sub_array;
		}
	...
	if($_POST["action"] == 'Add')
	{
		...
		{
			$error = '<div class="alert alert-danger">Examinee Data Already Exists</div>';
		}
		else
		{
			$student_image = '';
	...
	if($_POST["action"] == 'change_status')
	{
		...
		echo '<div class="alert alert-success">Examinee Status change to '.$_POST['next_status'].'</div>';
	}
